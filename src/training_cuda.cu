...
void run(){
    ...
    for (TI run_i = 0; run_i < NUM_RUNS; run_i++){
        ...
        std::cout << "CUDA Training took: " << std::chrono::duration_cast<std::chrono::duration<double>>(end - start).count() << "s" << std::endl;
    }
}

int main(void){
    std::cout << "Starting CUDA-accelerated RL training..." << std::endl;
